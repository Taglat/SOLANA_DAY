...
@router.post("/create-pictures")
async def create_picture_collection(db: Session = Depends(get_db)):
    """Создание коллекции картинок для сбора"""
    try:
        ...
        for picture_data in pictures_data:
            ...
            picture = NFTPuzzle(
                ...
                image_url=f"https://example.com/pictures/{picture_data['name']}.png",
                position_x=picture_data["position_x"],
                position_y=picture_data["position_y"],
                rarity=picture_data["rarity"],
                required_achievements={"price_tokens": picture_data["price_tokens"]},
                is_active=True
            )
    ...
    return [
        {
            ...
            "rarity": picture.rarity,
            "price_tokens": picture.required_achievements.get("price_tokens", 0) if picture.required_achievements else 0
        }
...
async def buy_picture(
    ...
    try:
        ...
        # TODO: Проверяем баланс токенов пользователя
        # Пока что просто создаем NFT
